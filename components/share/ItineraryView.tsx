  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 md:py-12">
      <div className="border-border bg-surface shadow-card overflow-hidden rounded-2xl border">
        ...
        <div
          className="text-primary-foreground relative overflow-hidden p-7 md:p-9"
          style={
            trip.coverImage
              ? {
        ...
        >
          ...
          <div className="relative">
            <p className="text-primary-foreground/80 font-sans text-[11px] tracking-[0.14em] uppercase">
              {dayCount} days · {trip.destination}
            </p>
            <h1 className="font-display mt-1.5 text-3xl leading-tight font-semibold md:text-4xl">
              {trip.name}
            </h1>
            <p className="text-primary-foreground/85 mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 font-sans text-xs">
        ...
        <div className="space-y-8 p-5 md:p-7">
          ...
          <section className="space-y-3">
            {days.map((day, i) => {
              ...
              return (
                <div
                  key={key}
                  className="border-border bg-canvas/40 break-inside-avoid rounded-lg border p-4"
                >
                  ...
                  {entries.length === 0 ? (
                    <p className="text-muted/70 mt-2 text-sm italic">Open day</p>
                  ) : (
          ...
          {trip.items.length > 0 && (
            <section>
              <h2 className="eyebrow text-muted mb-3">All places · {trip.items.length}</h2>
              <div className="grid gap-2 sm:grid-cols-2">
                {trip.items.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="border-border bg-canvas/30 flex items-center gap-2.5 rounded-md border px-3 py-2"
                    >
                      <span
                        className="flex size-7 shrink-0 items-center justify-center rounded-full"
                        style={{
        ...
        <footer className="border-border text-muted flex items-center justify-center gap-2 border-t py-4 font-sans text-[11px]">
...
function DayWeather({ fc, climate }: { fc?: ForecastDay; climate: ClimateSummary | null }) {
  if (fc) {
    ...
    return (
      <span className="bg-surface flex items-center gap-2 rounded-md px-2.5 py-1.5 text-right">
  ...
  if (climate) {
    return (
      <span
        className="bg-surface text-muted rounded-md px-2.5 py-1.5 text-right font-sans text-xs"
        title="Seasonal average"
      >
        <span className="text-ink">{climate.avgTMax}°</span> {climate.avgTMin}°
        <span className="ml-1 text-[10px] tracking-wide uppercase">avg</span>
      </span>
    );
  }
  return null;
