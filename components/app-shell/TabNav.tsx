...
import { navItems, PRIMARY_NAV_COUNT } from './nav-items';
...
/** Mobile: fixed bottom navigation (primary tabs + a "More" sheet). Desktop uses the Sidebar. */
export function BottomNav() {
  const pathname = usePathname();
  const [moreOpen, setMoreOpen] = useState(false);

  const primary = navItems.slice(0, PRIMARY_NAV_COUNT);
  const overflow = navItems.slice(PRIMARY_NAV_COUNT);
...
function Tab({ item, active }: { item: (typeof navItems)[number]; active: boolean }) {
