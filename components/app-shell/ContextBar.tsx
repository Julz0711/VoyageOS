...
export function ContextBar({
  ...
  return (
    <div className="border-border bg-canvas/85 sticky top-0 z-20 border-b pt-[env(safe-area-inset-top)] backdrop-blur-sm">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-wrap items-end justify-between gap-x-6 gap-y-2 px-5 py-3 md:px-12 md:py-5">
        <div className="min-w-0">
          {activeTrip && (
            <p className="eyebrow text-muted mb-1 flex items-center gap-1.5">
          ...
          <div className="relative">
            ...
            >
              <span className="font-display text-ink truncate text-2xl leading-tight font-semibold md:text-3xl">
            ...
            {open && (
              <div
                role="menu"
                className="border-border bg-surface text-ink shadow-lift absolute top-full left-0 z-30 mt-2 w-72 overflow-hidden rounded-lg border"
              >
                  ...
                  {trips.map((trip) => (
                    <li key={trip.id}>
                      <button
                        ...
                        className="hover:bg-canvas flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-sm"
                      >
                        ...
                        {activeTrip?.id === trip.id && (
                          <Check className="text-ink size-4 shrink-0" aria-hidden />
                        )}
                      </button>
                    </li>
                  ))}
                ...
                <Link
                  ...
                  className="border-border text-ink hover:bg-canvas flex items-center gap-2 border-t px-3 py-2.5 text-sm font-medium"
                >
        ...
        {countdown && (
          <span
            className={cn(
              'rounded-pill inline-flex items-center gap-2 border px-3 py-1 font-sans text-xs',
